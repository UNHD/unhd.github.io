...
渲染仅提交视口内及其边缘的标本，远处未选中花体使用同源曲线生成的简化几何，选中及附近标本保持完整精度。花体特效使用原像素密度的局部遮罩，保留信号来源的深度遮挡与抗锯齿；画面扭曲复用已有缓冲，不额外渲染一遍场景。场景离开视口、开场完全遮盖场景或减少动态效果下静止时暂停重复绘制。
...
`npm run check` 包含 34 项档案、模型、动画、渲染优化与开场加载顺序检查。修改 GLSL 或花体后处理时，在 Windows 上另行运行 `npm run check:shaders`。
